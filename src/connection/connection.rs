...
use crate::connection::{tls::TlsPreloginWrapper, tls_stream::create_tls_stream};
use crate::{
    EncryptionLevel, ProtocolReader,
    connection::{AuthMethod, Config, tls::MaybeTlsStream},
    protocol::{
        Context, HEADER_BYTES,
        pipeline::TokenStream,
        wire::{
            self, CompletionMessage, Encode, Frame, FrameCodec, Handshake, LoginRequest,
            PacketHeader, PacketStatus,
        },
    },
};
...
use winauth::{NextBytes, windows::NtlmSspiBuilder};
#[cfg(any(windows, feature = "integrated-auth-gssapi"))]
use wire::AuthChallenge;

/// A `Connection` is an abstraction between the [`Client`] and the server. It
/// can be used as a `Stream` to fetch [`Frame`]s from and to `send` packets
...
{
    transport: Framed<MaybeTlsStream<S>, FrameCodec>,
    flushed: bool,
    context: Context,
    buf: BytesMut,
}
...
impl<S: AsyncRead + AsyncWrite + Unpin + Send> Connection<S> {
    ...
    pub(crate) async fn connect(config: Config, tcp_stream: S) -> crate::Result<Connection<S>> {
        ...
        let transport = Framed::new(MaybeTlsStream::Raw(tcp_stream), FrameCodec);
    ...
    async fn flush_done(&mut self) -> crate::Result<CompletionMessage> {
        TokenStream::new(self).flush_done().await
    }

    #[cfg(any(windows, feature = "integrated-auth-gssapi"))]
    /// Flush the incoming token stream until receiving `SSPI` token.
    async fn flush_sspi(&mut self) -> crate::Result<AuthChallenge> {
    ...
    fn post_login_encryption(mut self, encryption: EncryptionLevel) -> Self {
        if let EncryptionLevel::Off = encryption {
            ...
            self.transport = Framed::new(MaybeTlsStream::Raw(tcp), FrameCodec);
        }
    ...
    pub(crate) async fn write_to_wire(
        ...
        let packet = Frame::new(header, data);
    ...
    async fn prelogin(
        ...
        let response: Handshake = wire::collect_from(self).await?;
            ...
            AuthMethod::Integrated => {
                ...
                match client.next_bytes(Some(sspi_bytes.as_ref()))? {
                    Some(sspi_response) => {
                        ...
                        let token = AuthChallenge::new(sspi_response);
            ...
            AuthMethod::Integrated => {
                ...
                let next_token = match ctx.step(Some(auth_bytes.as_ref()))? {
                    Some(response) => {
                        ...
                        AuthChallenge::new(Vec::from(response.deref()))
                    }
                    None => {
                        event!(Level::TRACE, response_len = 0);
                        AuthChallenge::new(Vec::new())
                    }
                };
    ...
    async fn tls_handshake(
        ...
        if encryption != EncryptionLevel::NotSupported {
            ...
            let transport = Framed::new(MaybeTlsStream::Tls(stream), FrameCodec);
...
impl<S: AsyncRead + AsyncWrite + Unpin + Send> ProtocolReader for Connection<S> {
