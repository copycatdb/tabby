...
pub struct Context {
    ...
    last_meta: Option<Arc<ColumnSchema<'static>>>,
    spn: Option<String>,
}
...
impl Context {
    ...
    pub fn set_last_meta(&mut self, meta: Arc<ColumnSchema<'static>>) {
        self.last_meta.replace(meta);
    }

    pub fn last_meta(&self) -> Option<Arc<ColumnSchema<'static>>> {
