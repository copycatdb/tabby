...
impl<'a> Encode<BytesMut> for LoginRequest<'a> {
...
mod tests {
    ...
    impl<'a> WireDecode<BytesMut> for LoginRequest<'a> {
        fn decode(src: &mut BytesMut) -> crate::Result<Self>
        where
            Self: Sized,
        {
            let mut cursor = Cursor::new(src);
            let mut ret = LoginRequest::new();
    ...
    fn login_message_round_trip() {
        ...
        let decoded = LoginRequest::decode(&mut payload).expect("decode should succeed");
