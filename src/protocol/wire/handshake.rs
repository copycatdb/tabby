...
pub struct Handshake {
    ...
    pub mars: bool,
    /// token=0x05
    pub activity_id: Option<TraceId>,
    /// token=0x06
...
impl Encode<BytesMut> for Handshake {
...
impl WireDecode<BytesMut> for Handshake {
    fn decode(src: &mut BytesMut) -> Result<Self>
    where
        Self: Sized,
    {
        let mut cursor = Cursor::new(src);
        let mut ret = Handshake::new();
...
mod tests {
    ...
    fn prelogin_roundtrip() {
        ...
        let prelogin = Handshake::new();
        prelogin
            .clone()
            .encode(&mut payload)
            .expect("encode should succeed");

        let decoded = Handshake::decode(&mut payload).expect("decode should succeed");
    ...
    fn prelogin_with_fedauth_roundtrip() {
        let mut payload = BytesMut::new();
        let mut prelogin = Handshake::new();
