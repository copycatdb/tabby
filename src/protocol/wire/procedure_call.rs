use super::{ALL_HEADERS_LEN_TX, AllHeaderTy, Encode};
use crate::{BytesMutWithTypeInfo, Result, protocol::wire::SqlValue};
...
pub struct ProcedureCall<'a> {
    ...
    params: Vec<ProcedureParam<'a>>,
    transaction_desc: [u8; 8],
}

impl<'a> ProcedureCall<'a> {
    pub fn new<I>(proc_id: I, params: Vec<ProcedureParam<'a>>, transaction_desc: [u8; 8]) -> Self
    where
...
impl<'a> Encode<BytesMut> for ProcedureCall<'a> {
...
impl<'a> Encode<BytesMut> for ProcedureParam<'a> {
    fn encode(self, dst: &mut BytesMut) -> Result<()> {
        let len_pos = dst.len();
        let mut length = 0u8;
