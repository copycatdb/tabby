...
from_sql!(
    NaiveDateTime:
        SqlValue::SmallDateTime(dt) => dt.map(|dt| NaiveDateTime::new(
            from_days(dt.days as i64, 1900),
            from_mins(dt.seconds_fragments as u32 * 60),
        )),
        SqlValue::DateTime2(dt) => dt.map(|dt| NaiveDateTime::new(
        ...
        SqlValue::DateTime(dt) => dt.map(|dt| NaiveDateTime::new(
            from_days(dt.days as i64, 1900),
            from_sec_fragments(dt.seconds_fragments as i64)
        ));
    NaiveTime:
        SqlValue::Time(time) => time.map(|time| {
    ...
    NaiveDate:
        SqlValue::Date(date) => date.map(|date| from_days(date.days() as i64, 1));
    chrono::DateTime<Utc>:
        SqlValue::DateTimeOffset(dto) => dto.map(|dto| {
        ...
        SqlValue::DateTime2(dt2) => dt2.map(|dt2| {
    ...
    chrono::DateTime<FixedOffset>: SqlValue::DateTimeOffset(dto) => dto.map(|dto| {
        let date = from_days(dto.datetime2.date.days() as i64, 1);
        let ns = dto.datetime2.time.increments as i64 * 10i64.pow(9 - dto.datetime2.time.scale as u32);
        let time = NaiveTime::from_hms_opt(0,0,0).unwrap() + chrono::Duration::nanoseconds(ns);
...
to_sql!(self_,
        NaiveDate: (SqlValue::Date, Date::new(to_days(*self_, 1) as u32));
        ...
        NaiveDateTime: (SqlValue::DateTime2, {
            use chrono::Timelike;
        ...
        chrono::DateTime<Utc>: (SqlValue::DateTime2, {
            use chrono::Timelike;
        ...
        chrono::DateTime<FixedOffset>: (SqlValue::DateTimeOffset, {
            use chrono::Timelike;
