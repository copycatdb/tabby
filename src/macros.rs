    ...
    ($target:ident, $( $ty:ty: ($variant:expr, $val:expr) ;)* ) => {
        $(
            impl crate::IntoSql for $ty {
                fn to_sql(&self) -> crate::protocol::wire::SqlValue<'_> {
                    let $target = self;
                    $variant(Some($val))
                }
            }
...
macro_rules! into_sql {
    ($target:ident, $( $ty:ty: ($variant:expr, $val:expr) ;)* ) => {
        $(
            impl<'a> crate::IntoSqlOwned<'a> for $ty {
                fn into_sql(self) -> crate::protocol::wire::SqlValue<'a> {
                    let $target = self;
                    $variant(Some($val))
                }
            }

            impl<'a> crate::IntoSqlOwned<'a> for Option<$ty> {
                fn into_sql(self) -> crate::protocol::wire::SqlValue<'a> {
                    match self {
                        Some(val) => {
                            let $target = val;
...
macro_rules! from_sql {
    ($( $ty:ty: $($pat:pat => ($borrowed_val:expr, $owned_val:expr)),* );* ) => {
        $(
            impl<'a> crate::FromServer<'a> for $ty {
                fn from_sql(data: &'a crate::protocol::wire::SqlValue<'static>) -> crate::Result<Option<Self>> {
                    match data {
                        $( $pat => Ok($borrowed_val), )*
            ...
            impl crate::FromServerOwned for $ty {
                fn from_sql_owned(data: crate::protocol::wire::SqlValue<'static>) -> crate::Result<Option<Self>> {
                    match data {
                        $( $pat => Ok($owned_val), )*
                        _ => Err(crate::Error::Conversion(format!("cannot interpret {:?} as an {} value", data, stringify!($ty)).into()))
                    }
